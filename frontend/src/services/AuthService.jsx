...
const AuthService = create((set) =>({
    ...
    //UPDATE PROFILE
    updateProfilePic : async (img) =>{
        set({isUpdatingProfile : true})

        try {
            const res = await client.put('/auth/update-profile-picture',img)
            set({authUser : res.data})
            ...
            const msg = error?.res?.data?.message || error.message || 'Error during  upload !';
            console.log('Error during image upload !' ,msg)
            return {
                success : false,
                message : msg
            }
